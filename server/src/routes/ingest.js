...
//     // ---- PDF Handling ----
//     if (ext === '.pdf') {
//       parsed = await parsePDF(req.file.path); // { pages, fullText }
...
//       const imgParsed = await parseImage(req.file.path); // { text, blocks }
...
//       tables.forEach((t, i) => {
//         tableChunks.push({ text: t, page: p.page, type: 'table' });
//       });
//     }

//     // ---- Chart Captions Placeholder ----
...
//     // ---- Embeddings ----
//     const vectors = await Promise.all(filtered.map(async (chunk, i) => {
//       const vec = await getEmbeddings(chunk.text);
//       return {
//         id: `${req.file.filename}-${chunk.page}-${i}`,
//         values: vec,
//         metadata: {
